...
import { snapPoint, transform2d } from "react-native-redash";
...
export const Card = ({ card: { source }, index, shuflleback }: CardProps) => {
  const x = useSharedValue(0)
  const y = useSharedValue(-height)
  const theta = Math.random() * 20 - 10
  const rotateZ = useSharedValue(0)
  const scale = useSharedValue(1)
  useAnimatedReaction(() => shuflleback.value, () => {
  ...
  useEffect(() =>{
    ...
    rotateZ.value =withDelay(delay, withTiming(
      theta, 
        {duration: DURATION,
        easing: Easing.inOut(Easing.ease)}
    ))  
